...
async function buscarProductoPorCodigoEnMensaje(mensaje, env) {
  try {
    ...
    for (const p of listado) {
      const fullKey = (p.key || "").toUpperCase();
      const idFromKey = fullKey.split(':').pop();
      const sku = (p.sku || "").toUpperCase();
      const codigo = sku || idFromKey;
...
async function callVendedorElitePro(message, env, productoActual, pideCompra, coverage, esSoloSaludo, esPrimerMensaje, yaEnvioMenu, esNuevoProducto) {
  ...
  const reglas = [
    ...
    "10. SALUDO: " + (esPrimerMensaje ? "Saluda cordialmente al inicio." : "NO saludes, ya estamos en una conversación.")
  ];
...
async function moduloCatalogo(message, state, env) {
  ...
  if (resultados.length === 0) return { text: "Lo siento, no encontré opciones de " + cat.toUpperCase() + " disponibles en este momento. 😉" };

  state.carrito_json = resultados.map(p => ({ key: p.key, nombre: p.nombre || p.titulo, precio: p.precio }));
  state.filtro_tamano = null;
...
<body class="flex flex-col h-screen overflow-hidden">
    ...
    <header class="p-6 flex items-center justify-between border-b border-gray-800 bg-[#0b0d11]">
        <div class="flex items-center gap-3">
            ...
            <div>
                <h1 class="font-bold text-lg">La Mueblería IA</h1>
                <p class="text-xs text-green-500 flex items-center gap-1"><span class="w-2 h-2 bg-green-500 rounded-full animate-pulse"></span> En línea</p>
            </div>
        </div>
    ...
    <footer class="glass p-4 md:p-6 pb-8">
        <div class="max-w-4xl mx-auto relative">
            <input type="text" id="user-input"
                ...
                class="w-full bg-[#2a2d35] border border-gray-700 rounded-2xl py-4 px-6 pr-14 focus:outline-none focus:ring-2 focus:ring-green-600 transition-all text-white"
                onkeypress="if(event.key === 'Enter') handleInput()">
            <button onclick="handleInput()" id="send-btn" class="absolute right-3 top-1/2 -translate-y-1/2 w-10 h-10 bg-green-600 hover:bg-green-500 rounded-xl flex items-center justify-center text-white transition-all">
    ...
    <script>
        ...
        function getChatState() {
            return JSON.parse(sessionStorage.getItem('chat_state') || '{}');
        }
        ...
        function appendMessage(text, isUser, images = []) {
            ...
            if (images && images.length > 0) {
                ...
                imgGrid.className = 'grid grid-cols-1 sm:grid-cols-2 gap-2 mt-3';
                images.forEach(url => {
                    const img = document.createElement('img');
                    img.src = url;
                    img.className = 'rounded-lg w-full h-48 object-cover cursor-pointer hover:opacity-90 transition-opacity';
        ...
        function showTyping() {
            ...
            div.className = 'message-bot p-4 flex gap-1 items-center';
        ...
        function handleInput() {
            ...
            // Client-side buffer (Debounce)
            if (bufferTimeout) clearTimeout(bufferTimeout);
            bufferTimeout = setTimeout(() => {
                sendConsolidatedMessage();
            }, 2500);
        }
        ...
        async function sendConsolidatedMessage() {
            ...
            const fullMessage = pendingMessages.join(' ');
            pendingMessages = [];
            bufferTimeout = null;
            ...
            try {
                ...
                removeTyping();
                if (data.text) {
                    appendMessage(data.text, false, data.images);
...
export default {
  async fetch(request, env, ctx) {
    ...
    if (request.method === "POST" && url.pathname === "/chat") {
      ...
      const result = await processFullFlow(message, state || {}, env);
