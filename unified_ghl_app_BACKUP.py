...
class App(cctk.CTk):
    ...
    def start_process(self):
        # Requirement: Process only modules that have a selected date range.
        ...
        if not (has_sales or has_contacts or has_fb): messagebox.showwarning("Atención", "Elija al menos un rango de fechas para generar el reporte."); return
        self.generate_btn.configure(state="disabled", text="🚀 PROCESANDO..."); threading.Thread(target=self.execute_logic, daemon=True, args=(has_sales, has_contacts, has_fb)).start()
        ...
        try:
            ...
            with ThreadPoolExecutor(max_workers=5) as ex:
                ...
                if has_sales:
                    ...
                    ghl_s_o, ghl_e_o = sd_opp.strftime("%Y-%m-%dT00:00:00.000Z"), ed_opp.strftime("%Y-%m-%dT23:59:59.999Z")
                    for acc in ACCOUNTS:
                        fut = ex.submit(fetch_for_account, acc, ghl_s_o, ghl_e_o, s_iso_o, e_iso_o, self.log)
                        futures.append(fut); f_map[fut] = ("opp", acc)
                if has_contacts:
                    ...
                    for acc in ACCOUNTS:
                        ...
                        futures.append(fut); f_map[fut] = ("con", acc)
                if has_fb:
                    ...
                    for acc in FB_AD_ACCOUNTS:
                        ...
                        futures.append(fut); f_map[fut] = ("fb", acc)
                    ...
                    if type == "opp": o, v = f.result(); res_o.extend(o); res_v.extend(v)
                        ...
                        if insights:
                            ...
                            for ins in insights:
                                ...
                                if is_usd: spend_q, budget_q, spend_usd, budget_usd = spend_orig * USD_TO_GTQ, budget_orig * USD_TO_GTQ, spend_orig, budget_orig
                                else: spend_q, budget_q, spend_usd, budget_usd = spend_orig, budget_orig, spend_orig / USD_TO_GTQ, budget_orig / USD_TO_GTQ
                                res_fb.append({"ID del anuncio": aid, "ID de la página": pid, "Nombre de la página": pname, "Nombre de la campaña": camp, "Nombre del conjunto": ins.get("adset_name"), "Nombre del anuncio": ad_n, "Día": ins.get("date_start"), "Contactos mensajes nuevos": conv, "Importe gastado Q": spend_q, "Presupuesto Q": budget_q, "Importe gastado $": spend_usd, "Presupuesto $": budget_usd, "Inicio informe": ins.get("date_start"), "Fin informe": ins.get("date_stop"), "codigo": anu, "precio": extraer_precio_fb(ad_n), "tipo_post": tpost, "SECUENCIA": extraer_secuencia(camp)})
    ...
    def generate_excel(self, res_o, res_v, res_c, res_fb):
        ...
        self.log(f"¡EXITO! {fn}"); messagebox.showinfo("ÉXITO", f"Excel generado:\n{fn}")
